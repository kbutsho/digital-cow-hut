...
const createOrder = async (data: IOrder): Promise<IOrder> => {
  ...
  if (cow.price > buyer.budget) {
    throw new ApiError(httpStatus.BAD_REQUEST, 'budget is not enough to buy this cow!');
  }
  await User.findOneAndUpdate({ _id: cow.seller }, { income: seller.income + cow.price });
  await User.findOneAndUpdate({ _id: data.buyer }, { budget: buyer.budget - cow.price });
